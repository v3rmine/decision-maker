...
class ErrorAnalyzer:
    ...
    def check_errors_argument(self, frame_element_t, frame_element_p):
        ...
        if(frame_element_t['in_text']):
            self.TOT_ARGUMENT += 1
            if(isinstance(frame_element_t['argument'], str) and isinstance(frame_element_p['argument'], str) and frame_element_t['argument'].upper() != frame_element_p['argument'].upper()):
                self.ARGUMENT_C += 1
                argument_error = "TEXT"
